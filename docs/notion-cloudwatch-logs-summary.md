...
[EBS 파일] WAS: /opt/tomcat/logs/*   ─┤─ CloudWatch Agent(각 서버에 설치) ──▶ CloudWatch Logs  (계정에 1개 · VPC 밖 · 자체 저장소)
            SSM 세션                  ─┘                                       로그 그룹 /mc/web/* /mc/was/* /mc/ssm/sessions
...
3. **was-a 장애 발견·복구** — 알람 `mc-was-unhealthy-host`가 ALARM → `/mc/was/catalina`에서 `Access denied for user 'petclinic_app'` 확인(Proxy 인증 목록 반영 전 부팅) → Tomcat 재시작으로 healthy → `was.sh` 보강(`2f746d8`: Proxy 경유 앱 로그인 대기 + 404면 재시작). **서버에 들어가지 않고 CloudWatch Logs로 원인을 찾은 실제 사례.**
...
aws logs tail /mc/was/catalina --since 1h --profile mc-deploy | grep -iE "SEVERE|Access denied"
