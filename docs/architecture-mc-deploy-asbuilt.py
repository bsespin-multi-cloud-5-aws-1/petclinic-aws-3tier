...
d.svc("cwlogs", "CloudWatch Logs (계정에 1개 · VPC 밖)", "/petclinic/web/* · /petclinic/was/* 30일 · /petclinic/bastion/secure 90일<br><b>/aws/rds/instance/mc-petclinic/{error,slowquery} · /aws/rds/proxy/mc-rds-proxy</b> 30일<br>스트림 = 인스턴스 ID · 자체 저장소 · KMS · WAF 는 us-east-1", "cloudwatch", "ops", 2200, 190, w=280, h=125)
...
d.svc("cwparam", "Parameter Store (Agent 설정)", "/petclinic/cwagent/web · was · bastion<br>Agent 는 user_data 설치 · Session Manager 아님(enable_ssm=false)", "systems_manager", "ops", 2495, 595, w=245, h=115)
d.svc("iam", "IAM mc-ec2-role + mc-ec2-inline", "CW Agent · GetSecretValue(admin + app-db) · SSM Core 없음<br>kms:Decrypt(ViaService) · ssm:GetParameter(/petclinic/cwagent/*)<br>s3:PutObject/ListBucket mc-logs · CompleteLifecycleAction", "identity_and_access_management", "sec", 2200, 730, w=280, h=125)
