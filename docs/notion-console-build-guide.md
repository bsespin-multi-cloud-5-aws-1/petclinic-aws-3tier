...
4. **Kinesis → Data Firehose → 스트림 생성** ×4 (`mc-cwlogs-web` `mc-cwlogs-was` `mc-cwlogs-bastion` `mc-cwlogs-db`): 소스 **Direct PUT** · 대상 **S3** `mc-logs-528821350786` · 버퍼 5MiB / 300초 · 압축 GZIP · **레코드 변환: "압축 해제(Decompression, GZIP)" + "레코드 구분자 추가(개행)"** · 역할 `mc-firehose-cwlogs-role`. 접두사(tier 는 web·was·bastion·db):

```text
S3 접두사      cwlogs/<tier>/!{timestamp:yyyy/MM/dd}/
오류 접두사    cwlogs-errors/<tier>/!{firehose:error-output-type}/!{timestamp:yyyy/MM/dd}/
```
