...
| JDK | 컴파일 1.8 / CI 11 → Phase 1은 **Corretto 11** | **Corretto 17 (LTS, OpenJDK 빌드)** | Spring 5.3.39가 17 지원, AL2023에 `java-17-amazon-corretto-headless` | 안 뜨면 `java.version`을 11로 내리고 사유 기록(후퇴 경로) |
...
| wro4j (LESS 빌드 플러그인) | 1.8.0 | 1.10.1 | JDK 17에서 빌드 통과 위해 | 빌드 실패 시 첫 용의자 |
| JaCoCo | 0.8.6 | 0.8.12 | 0.8.6은 Java 16까지만 | 테스트 실행 시에만 관여 |
| JAXB runtime | 2.3.4 | 2.3.9 | 패치 | — |
| Maven (래퍼) | 3.5.4 | 3.9.16 | 2018년판 Maven은 JDK 17에서 Guice 반사 접근 경고·오류 가능 | `.mvn/wrapper/maven-wrapper.properties` 한 줄, 래퍼 jar은 그대로 |
| 배포 방식 | `tomcat7:deploy` (manager 앱, `tomcat/tomcat` 평문 계정) | WAR 파일 복사(→ 이후 S3 → WAS pull), **manager 앱 삭제** | 관리 콘솔 노출·평문 계정 제거 | — |
| DB 접속정보 | pom.xml에 하드코딩 | 빌드는 `-P MySQL` 한 번, 값은 **`-Djdbc.url/username/password` 런타임 주입** | `datasource-config.xml`의 `system-properties-mode="OVERRIDE"` 덕분에 가능. 비밀을 저장소에 안 남김 | 8일차에 Secrets Manager/RDS Proxy와 결합 |
